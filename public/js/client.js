...
function bake(img) {
  ...
  // 2) Альфа: фон -> 0, фигура -> 255. 
  // ИСПРАВЛЕНО: Меняем альфу только для пикселей, которые были помечены как фон (bg[j] === 1).
  // Если алгоритм Flood fill определил, что это часть персонажа, оставляем пиксель полностью непрозрачным.
  for (let i = 0, j = 0; i < p.length; i += 4, j++) {
    const m = lum(i);
    if (bg[j]) {
      p[i + 3] = m <= 8 ? 0 : m >= 40 ? 255 : Math.round(((m - 8) / 32) * 255);
      if (m <= FLOOR) p[i + 3] = 0; 
    } else {
      p[i + 3] = 255;
    }
