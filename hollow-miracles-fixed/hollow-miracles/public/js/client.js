...
function bake(img) {
  ...
  const p = d.data;
  const lum = i => Math.max(p[i], p[i + 1], p[i + 2]);

  // 1) Flood fill фона от 4 углов: всё, что связно с краем и достаточно тёмное — фон.
  //    Так удаляется ТОЛЬКО реальный чёрный фон вокруг фигуры, а тёмные части
  //    самой модели (одежда, броня) остаются полностью непрозрачными.
  const N = W * H;
  const bg = new Uint8Array(N);            // 1 = фон
  const stack = [];
  const FLOOR = 42;                        // порог «черноты фона»
  const seed = (px, py) => { const idx = py * W + px; if (!bg[idx] && lum(idx * 4) <= FLOOR) { bg[idx] = 1; stack.push(idx); } };
  for (let px = 0; px < W; px++) { seed(px, 0); seed(px, H - 1); }
  for (let py = 0; py < H; py++) { seed(0, py); seed(W - 1, py); }
  while (stack.length) {
    const idx = stack.pop();
    ...
    const tryN = (nx, ny) => {
      ...
      const ni = ny * W + nx;
      if (bg[ni]) return;
      if (lum(ni * 4) <= FLOOR) { bg[ni] = 1; stack.push(ni); }
    };
    tryN(px - 1, py); tryN(px + 1, py); tryN(px, py - 1); tryN(px, py + 1);
  }

  // 2) Альфа: фон -> 0, фигура -> 255. Мягкий край только на очень тёмных пикселях
  //    границы (0..15), всё остальное — полностью непрозрачно. Так тёмная одежда,
  //    броня и волосы персонажей не становятся прозрачными во время игры.
  for (let i = 0, j = 0; i < p.length; i += 4, j++) {
    if (bg[j]) { p[i + 3] = 0; continue; }
    const m = lum(i);
    p[i + 3] = m <= 2 ? 0 : m >= 15 ? 255 : Math.round(((m - 2) / 13) * 255);
  }
  x.putImageData(d, 0, 0);

  // 3) Авто-кроп по непрозрачному: убирает пустые поля, центрует фигурку.
  let minX = W, minY = H, maxX = 0, maxY = 0, any = false;
  for (let py = 0; py < H; py++) for (let px = 0; px < W; px++) {
    if (p[(py * W + px) * 4 + 3] > 24) { any = true; if (px < minX) minX = px; if (px > maxX) maxX = px; if (py < minY) minY = py; if (py > maxY) maxY = py; }
  }
...
function frame() {
  ...
  for (const l of (s.extras || [])) {
    ...
    for (let w = 0; w < 4; w++) {
      ...
      ctx.beginPath();
      ctx.moveTo(l.x, l.y);
      ctx.quadraticCurveTo(l.x + Math.cos(a) * 8, l.y + Math.sin(a) * 8, l.x + Math.cos(a) * 16, l.y + Math.sin(a + 0.5) * 14);
      ctx.stroke();
    }
    if (l.awakened) { ctx.strokeStyle = `rgba(${col},0.5)`; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.arc(l.x, l.y, 17 + Math.sin(now / 180) * 2, 0, 7); ctx.stroke(); }
  }

  // --- снаряды ---
  for (let i = 0; i < (s.extras || []).length; i++) {
    const p = s.extras[i];
    if (p.kind !== 'proj') continue;
    const key = p.team + ':' + i;
    const prev = projTrail.get(key);
    ctx.strokeStyle = 'rgba(255,233,176,0.8)'; ctx.lineWidth = 3;
    ...
    projTrail.set(key, { x: p.x, y: p.y });
  }
  if (!(s.extras || []).some(e => e.kind === 'proj')) projTrail.clear();
  ...
  for (let i = fx.length - 1; i >= 0; i--) {
    ...
    if (e.t === 'p') {
      e.x += e.vx * sdt; e.y += e.vy * sdt; e.vx *= 0.92; e.vy *= 0.92;
      ctx.globalAlpha = k;
      ctx.fillStyle = e.color;
      ctx.fillRect(e.x - e.size / 2, e.y - e.size / 2, e.size, e.size);
      ctx.globalAlpha = 1;
    } else if (e.t === 'r') {
      const r = lerp(e.r1, e.r0, k);
      ctx.globalAlpha = k; ctx.strokeStyle = e.color; ctx.lineWidth = e.lw * k + 0.5;
      ctx.beginPath(); ctx.arc(e.x, e.y, r, 0, 7); ctx.stroke();
      ctx.globalAlpha = 1;
    } else if (e.t === 'a') {
      ctx.globalAlpha = k; ctx.strokeStyle = e.color; ctx.lineWidth = e.lw * k + 1;
      ctx.beginPath(); ctx.arc(e.x, e.y, e.range, e.face - e.arc / 2, e.face + e.arc / 2); ctx.stroke();
      ctx.globalAlpha = 1;
    } else if (e.t === 'c') {
      ctx.globalAlpha = k * 0.45; ctx.fillStyle = e.color;
      ctx.beginPath(); ctx.moveTo(e.x, e.y);
      ctx.arc(e.x, e.y, e.range, e.face - e.arc / 2, e.face + e.arc / 2);
      ctx.closePath(); ctx.fill();
      ctx.globalAlpha = 1;
    } else if (e.t === 'b') {
      const hgt = 250, wdt = 8 + 28 * (1 - k);
      const g = ctx.createLinearGradient(e.x, e.y - hgt, e.x, e.y);
      g.addColorStop(0, 'rgba(255,233,176,0)');
      g.addColorStop(1, `rgba(255,233,176,${0.50 * k})`);
      ctx.fillStyle = g;
      ctx.fillRect(e.x - wdt / 2, e.y - hgt, wdt, hgt);
      ctx.fillStyle = `rgba(255,250,230,${0.85 * k})`;
      ctx.fillRect(e.x - 1.5, e.y - hgt, 3, hgt);
    } else if (e.t === 'beam') {
      const ex = e.x + Math.cos(e.ang) * e.len, ey = e.y + Math.sin(e.ang) * e.len;
      ctx.globalAlpha = k;
      ctx.strokeStyle = e.hit ? 'rgba(255,120,120,0.9)' : 'rgba(255,180,180,0.7)';
      ctx.lineWidth = (e.hit ? 6 : 3) * k + 1;
      ctx.beginPath(); ctx.moveTo(e.x, e.y); ctx.lineTo(ex, ey); ctx.stroke();
      ctx.strokeStyle = `rgba(255,255,255,${0.8 * k})`; ctx.lineWidth = 1.5;
      ctx.beginPath(); ctx.moveTo(e.x, e.y); ctx.lineTo(ex, ey); ctx.stroke();
      ctx.globalAlpha = 1;
    }
  }
  ...
  // --- оверлеи экрана ---
  const W = arena.w * scale, H = arena.h * scale;
  // туман врага
  if (zn.fog && zn.fog.team !== youTeam) {
    ctx.fillStyle = 'rgba(60,92,118,0.30)';
    ctx.fillRect(0, 0, W, H);
    for (let i = 0; i < 3; i++) {
      const gx = W * (0.25 + 0.25 * i) + Math.sin(now / 1700 + i * 2) * 90 * scale;
      const gy = H * 0.5 + Math.cos(now / 2100 + i) * 70 * scale;
      const g = ctx.createRadialGradient(gx, gy, 10, gx, gy, 260 * scale);
      g.addColorStop(0, 'rgba(110,150,180,0.16)'); g.addColorStop(1, 'rgba(110,150,180,0)');
      ctx.fillStyle = g; ctx.fillRect(0, 0, W, H);
    }
    // из тумана видны лишь розовые искры
    for (let i = 0; i < 16; i++) {
      const sx = ((i * 131 + 47) % 997) / 997 * W;
      const sy = ((i * 271 + 89) % 883) / 883 * H + Math.sin(now / 760 + i) * 9 * scale;
      const tw = 0.5 + 0.5 * Math.sin(now / 170 + i * 2.1);
      ctx.fillStyle = `rgba(232,77,122,${0.08 + 0.28 * tw})`;
      ctx.fillRect(sx, sy, 2.6 * scale, 2.6 * scale);
    }
  }
  // слепота
  const meP = youTeam === 'A' ? A : B;
  if (meP?.st?.blind) {
    const cx = meP.x * scale, cy = meP.y * scale;
    const g = ctx.createRadialGradient(cx, cy, 200 * scale, cx, cy, 340 * scale);
    g.addColorStop(0, 'rgba(0,0,0,0)'); g.addColorStop(1, 'rgba(0,0,0,0.94)');
    ctx.fillStyle = g; ctx.fillRect(-20, -20, W + 40, H + 40);
    ctx.strokeStyle = `rgba(163,59,59,${0.28 + 0.16 * Math.sin(now / 190)})`;
    ctx.lineWidth = 3 * scale;
    ctx.beginPath(); ctx.arc(cx, cy, 262 * scale, 0, 7); ctx.stroke();
  }
  // красная кромка при низком HP
  if (meP && meP.hp !== null && meP.hp / meP.maxHp < 0.25) {
    const g = ctx.createRadialGradient(W / 2, H / 2, H * 0.35, W / 2, H / 2, H * 0.72);
    g.addColorStop(0, 'rgba(142,47,47,0)'); g.addColorStop(1, `rgba(142,47,47,${0.18 + 0.1 * Math.sin(now / 240)})`);
    ctx.fillStyle = g; ctx.fillRect(0, 0, W, H);
  }
  // вспышки
  if (flashWhite > 0) { ctx.fillStyle = `rgba(255,255,255,${flashWhite})`; ctx.fillRect(0, 0, W, H); flashWhite -= 2.2 * dt; }
  if (flashRed > 0) { ctx.fillStyle = `rgba(142,47,47,${flashRed * 0.5})`; ctx.fillRect(0, 0, W, H); flashRed -= 1.6 * dt; }
  if (vinC) ctx.drawImage(vinC, 0, 0);
}

function drawFighter(f, now) {
  const meta = chars?.[f.id] || {};
  const isMine = f.team === youTeam;
  const r = f.r || 26;
  const dep = 0.9 + 0.22 * clamp((f.y || 0) / (arena.h || 700), 0, 1);  // дальше — мельче, ближе — крупнее

  // --- анимационное состояние ---
  const an = anim(f.eid);
  const dtv = an.pt ? Math.min(0.06, (now - an.pt) / 1000) : 0;
  const dxv = dtv > 0 ? (f.x - an.px) / dtv : 0;
  const dyv = dtv > 0 ? (f.y - an.py) / dtv : 0;
  const spd = Math.hypot(dxv, dyv);
  an.px = f.x; an.py = f.y; an.pt = now;
  an.vx = lerp(an.vx, dxv, 0.25);
  const moving = f.alive && spd > 34;
  if (moving) an.walkPh += dtv * Math.min(spd, 340) * 0.05;

  // --- смерть: угасание и оседание ---
  let deathK = 0;
  if (!f.alive) {
    deathK = an.dieAt ? clamp((now - an.dieAt) / 1100, 0, 1) : 1;
    if (deathK >= 1) return;
  }

  let alpha = 1 - deathK * 0.92;
  if (f.ghost) alpha = 0.12;
  else if (isMine && snapCur.zones?.fog && snapCur.zones.fog.team === youTeam && f.id === 'veil') alpha = Math.min(alpha, 0.8);

  // пыль из-под ног
  if (moving && now - an.dustAt > 170) {
    an.dustAt = now;
    fx.push({ t: 'p', x: f.x - dxv * 0.04 + (Math.random() - 0.5) * 8, y: f.y + r * 0.7, vx: -dxv * 0.12, vy: -8 - Math.random() * 14, life: .4, max: .4, size: 2.2, color: 'rgba(160,140,110,0.5)' });
  }

  // тень и подсветка (с угасанием при смерти)
  ctx.globalAlpha = 0.5 * alpha;
  ctx.fillStyle = '#000';
  ctx.beginPath(); ctx.ellipse(f.x, f.y + r * 0.82, r * 1.15 * dep * (1 - deathK * 0.4), r * 0.4 * dep, 0, 0, 7); ctx.fill();
  ctx.globalAlpha = alpha;

  if (!deathK) {
    const ringCol = isMine ? 'rgba(201,168,106,' : 'rgba(142,47,47,';
    const pul = 0.5 + 0.5 * Math.sin(now / 300);
    ctx.strokeStyle = ringCol + (0.35 + 0.25 * pul) + ')';
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.ellipse(f.x, f.y + r * 0.82, r * 1.3 * dep, r * 0.46 * dep, 0, 0, 7); ctx.stroke();
  }

  if (f.st?.invuln) {
    ctx.strokeStyle = `rgba(255,255,255,${0.3 + 0.3 * pul})`;
    ctx.beginPath(); ctx.arc(f.x, f.y - r * 0.6, r * 1.7, 0, 7); ctx.stroke();
  }

  // спрайт: поза по событиям + кадр удара по тактам
  let pose = 'idle';
  const ps = poseUntil.get(f.eid);
  if (ps && now < ps.until) pose = ps.pose;
  if (f.st?.channel) pose = 'atk';
  if (deathK) pose = 'idle';
  const set = SPRITES[f.id];
  let spr = null;
  if (set) {
    if (pose === 'atk') {
      const arr = set.atkArr.filter(Boolean);
      spr = arr.length ? arr[an.atkIx % arr.length] : (set.idle || null);
    } else spr = set[pose] || set.idle || set.atkArr.filter(Boolean)[0];
  }
  if (spr) {
    let h = r * 5.0 * dep;
    let w = h * (spr.w / spr.h);
    const maxW = r * 7.6 * dep;                       // широкие кадры (деш) не раздуваем
    if (w > maxW) { const k2 = maxW / w; w *= k2; h *= k2; }
    const flip = Math.cos(f.face) < -0.12;
    const fs = flip ? -1 : 1;

    // шлейф деша: призраки позади
    if (pose === 'dash') {
      an.trail.push({ x: f.x, y: f.y, t: now });
      if (an.trail.length > 12) an.trail.shift();
    }
    for (const lag of [150, 80]) {
      let g0 = null;
      for (const tr of an.trail) if (now - tr.t >= lag) g0 = tr;
      if (g0 && now - g0.t < 360 && (pose === 'dash' || now - g0.t < 220)) {
        ctx.save();
        ctx.globalAlpha = alpha * (lag === 150 ? 0.10 : 0.20);
        ctx.translate(g0.x, g0.y + r * 0.9);
        if (flip) ctx.scale(-1, 1);
        ctx.drawImage(spr.c, -w / 2, -h, w, h);
        ctx.restore();
      }
    }
    if (pose !== 'dash' && an.trail.length && now - an.trail[an.trail.length - 1].t > 380) an.trail.length = 0;

    let jx = 0, jy = 0, rot = 0, sx = 1, sy = 1;
    if (f.st?.channel) { jx = (Math.random() - 0.5) * 3; jy = (Math.random() - 0.5) * 3; }
    if (f.alive && !moving && pose === 'idle') sy *= 1 + 0.018 * Math.sin(now / 430 + f.eid * 1.7);          // дыхание
    if (moving) { jy -= Math.abs(Math.sin(an.walkPh)) * r * 0.10; rot += clamp(an.vx / 320, -1, 1) * 0.05 * fs; } // шаг и наклон
    if (pose === 'atk' && ps && ps.t0) {
      const pt = clamp((now - ps.t0) / Math.max(60, ps.until - ps.t0), 0, 1);
      if (ps.phase === 'wind') {                                  // замах: отвод назад, поджатие
        jx -= Math.cos(f.face) * 6 * pt;
        jy += Math.sin(f.face) * 3 * pt;
        sx *= 1 - 0.05 * pt; sy *= 1 + 0.04 * pt;
      } else {                                                    // контакт: выброс вперёд + squash-stretch
        const k = 1 - pt;
        jx += Math.cos(f.face) * 8 * k;
        jy += Math.sin(f.face) * 4 * k;
        const ss = 1 + 0.13 * k;
        sx *= ss; sy *= ss * 0.95 + 0.05;
      }
    }
    if (pose === 'dash') sx *= 1.06;
    if (deathK) { jy += deathK * r * 1.15; rot += fs * deathK * 0.55; }                                        // оседание

    ctx.save();
    ctx.translate(f.x + jx, f.y + r * 0.9 + jy);
    if (flip) ctx.scale(-1, 1);
    ctx.rotate(rot);
    ctx.scale(sx, sy);
    if (f.st?.nullified || deathK) { try { ctx.filter = `grayscale(${Math.max(deathK, f.st?.nullified ? 0.85 : 0)}) brightness(${0.9 - deathK * 0.3})`; } catch (e) {} }
    ctx.drawImage(spr.c, -w / 2, -h, w, h);
  ...
  if (deathK) { ctx.globalAlpha = 1; return; }

  // статусы
  if (f.st?.channel) {
    const k = 0.5 + 0.5 * Math.sin(now / 90);
    ctx.strokeStyle = `rgba(255,255,255,${0.5 + 0.4 * k})`;
    ctx.lineWidth = 3;
    ctx.beginPath(); ctx.arc(f.x, f.y - r, r * (1.4 + k * 0.7), 0, 7); ctx.stroke();
  }
  if (f.st?.stun) {
    for (let i = 0; i < 3; i++) {
      const a = now / 260 + i * 2.09;
      ctx.fillStyle = '#f0dca0';
      ctx.fillRect(f.x + Math.cos(a) * 18 - 2, f.y - r * 4.3 + Math.sin(a) * 6 - 2, 4, 4);
    }
  }
  if (f.st?.poisoned) {
    ctx.fillStyle = 'rgba(143,209,122,0.8)';
    const a = now / 320;
    ctx.fillRect(f.x + Math.cos(a) * 14, f.y - r * 2 + ((now / 14) % (r * 2.4)) - r * 2.4, 3, 3);
  }
  if (f.st?.crack) {
    ctx.strokeStyle = 'rgba(240,160,80,0.9)'; ctx.lineWidth = 1.6;
    ctx.beginPath();
    ctx.moveTo(f.x - 8, f.y - r * 2.4); ctx.lineTo(f.x - 2, f.y - r * 2.0); ctx.lineTo(f.x - 7, f.y - r * 1.6);
    ctx.stroke();
  }
  if (f.isReal === true) {
    ctx.fillStyle = '#c9a86a';
    ctx.beginPath();
    ctx.moveTo(f.x, f.y - r * 4.9); ctx.lineTo(f.x + 5, f.y - r * 4.6); ctx.lineTo(f.x, f.y - r * 4.3); ctx.lineTo(f.x - 5, f.y - r * 4.6);
    ctx.closePath(); ctx.fill();
  }

  // мини-полоска HP
  if (f.alive && f.hp !== null) {
    const bw = 48, bx = f.x - bw / 2, by = f.y - r * 4.1;
    ctx.fillStyle = 'rgba(0,0,0,0.65)';
    ctx.fillRect(bx - 1, by - 1, bw + 2, 6);
    ctx.fillStyle = isMine ? '#b8413f' : '#8e2f2f';
